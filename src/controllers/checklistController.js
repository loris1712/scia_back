  ...
  try {
    ...
    const maintenanceTypes = await recurrencyType.findAll({
      where: {
        id: [
          ...
          "3", //Manutenzioni annuali
          "4", //Manutenzioni extra
        ],
      },
    });
    ...
    const formattedData = maintenanceTypes.map((type) => {
      return {
        id: type.id,
        title: type.name,
        length: tasks.filter((task) => task.recurrenceTypeId === type.id).length,
        tasks: tasks
          .filter((task) => task.recurrenceTypeId === type.id)
          .map((task) => ({
            id: task.id,
            ...
            element: task.element
              ? {
                  ...
                  serial_number: task.element.serial_number,
                  progressive_code: task.element.progressive_code,
                  installation_date: task.element.installation_date,
                }
              : null,
